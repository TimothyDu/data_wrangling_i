...
\##renaming columns, using select function, we get to keep what columns
we want and rename them
...
\##in dplyer there is a delicated function used for renaming, rename
function, this way we do not delete any columns, just rename
...
?select_helpers we are interested in funtions that let us select colums
that i want to keep based on some criteria
...
\##logical operator we want to ask if gd_of_birth equals to 20, == tests
for equality, we are not giving gd_of_birth a value of 20, just test if
gd_of_birth equals to 22 is true or not
...
the code chunk below will give me everything that gd_of_birth not equals
20 another way to do
...
\##`arrange` put things in order we arrange the the data frame by
pups_born_alive varaible from lowest to highest
